...
class M_comment extends CI_Model
{
  ...
  {
    $data = $this->db->query("select * from user natural join comment order by timestamp desc")->result_array();
    return $data;
  }
  public function showCommDetail($comm_id)
  {
    $data = $this->db->query("select * from user natural join comment where comm_id=$comm_id")->result_array()[0];
    return $data;
  }
